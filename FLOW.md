...
> ⚠️ **이 다이어그램은 랩이 고른 방식이지, 확정된 실제 운영 계획이 아니다.**
> `superapp-proxy`↔`gateway`의 mTLS는 **gateway 자신이 직접 종료**하는 방식이다 —
> 뒤에 나올 세 가지 실제 후보(L4 로드밸런서/Envoy Gateway/gateway 자신) 중 하나를
> 로컬에서 증명한 것뿐이다. 실제 next.msa 아키텍처에서 어디서 종료할지는 아직 정해지지
> 않았다. 자세한 비교는 11절과 **`MTLS-REVIEW.md`**("mTLS 검토안" 탭) 참고.
